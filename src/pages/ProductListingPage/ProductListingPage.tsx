...
const areStringArraysEqual = (a: string[], b: string[]) => a.length === b.length && a.every((value, index) => value === b[index]);
...
const parseFiltersFromSearchParams = (searchParams: URLSearchParams): ProductFilters => {
  ...
  const minPrice = Number.isFinite(minPriceParam) && minPriceParam > 0 ? minPriceParam : 0;
  const maxPriceValue = Number.isFinite(maxPriceParam) && maxPriceParam > 0 ? maxPriceParam : Number.POSITIVE_INFINITY;

  const clampedMin = Number.isFinite(maxPriceValue) ? Math.min(minPrice, maxPriceValue) : minPrice;
...
const ProductListingPage = () => {
  const { products, filteredProducts, filters, setFilters, isLoading } = useProducts();
  ...
  const priceIsActive =
    filters.priceRange[0] > 0 || (Number.isFinite(filters.priceRange[1]) && filters.priceRange[1] < maxPrice);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [filters.categories.length, filters.inStockOnly, filters.query, priceIsActive]);
  ...
  const activeFilterLabels = useMemo(() => {
    ...
    if (priceIsActive) {
      const maxDisplay = Number.isFinite(filters.priceRange[1]) ? filters.priceRange[1] : maxPrice;
      labels.push(`Price: ${formatPrice(filters.priceRange[0])} - ${formatPrice(maxDisplay)}`);
    }

    return labels;
  }, [filters.categories, filters.inStockOnly, filters.priceRange, filters.query, maxPrice, priceIsActive]);
      ...
      <div className={styles['page__header']}>
        <div>
          ...
          {activeFilterLabels.length > 0 ? (
            <div className={styles['page__active-filters']} aria-label="Active filters">
              {activeFilterLabels.map((label) => (
                <Badge key={label} size="sm" className={styles['page__active-filter']}>
                  {label}
                </Badge>
              ))}
            </div>
