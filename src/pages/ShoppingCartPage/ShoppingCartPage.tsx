  ...
  return (
    <div className={styles.page}>
      <header className={styles['page__header']}>
        ...
        <p className={styles['page__subtitle']}>Secure checkout. Complimentary insured shipping worldwide.</p>
      </header>
      {items.length === 0 ? (
        ...
        <div className={styles['page__content']}>
          <div className={styles['page__items']}>
            {items.map((item) => (
              <CartItem key={`${item.productId}-${createVariantKey(item.variant)}`} item={item} />
            ))}
          </div>
