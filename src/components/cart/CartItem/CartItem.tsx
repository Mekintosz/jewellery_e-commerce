...
export const CartItem = ({ item }: CartItemProps) => {
  ...
  return (
    <article className={styles['cart-item']}>
      <img src={item.image} alt={item.name} className={styles['cart-item__image']} loading="lazy" />
      <div className={styles['cart-item__details']}>
        ...
        <div className={styles['cart-item__controls']}>
          <div className={styles['cart-item__quantity']}>
            <button
              type="button"
              onClick={() => updateQuantity(item.productId, key, item.quantity - 1)}
          ...
          <button type="button" className={styles['cart-item__remove']} onClick={() => removeItem(item.productId, key)}>
            Remove
          </button>
        </div>
      </div>
      <div className={styles['cart-item__pricing']}>
        <p className={styles['cart-item__price']}>{formatCurrency(item.salePrice ?? item.price)}</p>
      </div>
    </article>
  );
};
