...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, helperText, error, className, id, children, ...props }, ref) => (
    <div className={clsx(styles.select, className)}>
      ...
      >
        <select ref={ref} id={id} className={styles['select__field']} {...props}>
          {children}
        </select>
      ...
      {error ? <span className={styles['select__error']} role="alert">{error}</span> : null}
      {helperText && !error ? <span className={styles['select__helper']}>{helperText}</span> : null}
    </div>
  )
);
